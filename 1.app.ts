...
const { validateMovie, validatePartialMovie } = require("./schemas/movies.ts");
...
app.get("/movies", (req: any, res: any) => {
    ...
    if(genre) {
        const filteredMovies = movies.filter((movie: any) => movie.genre.some((g: any) => g.toLowerCase() === genre.toLowerCase()));
...
app.post("/movies", (req: any, res: any) => {
...
app.patch("/movies/:id", (req: any, res: any) => {
    const { id } = req.params;
...
app.delete("/movies/:id", (req: any, res: any) => {
    const origin = req.header("origin");
    if (ACCEPTED_ORIGINS.includes(origin) || !origin) {
        res.header("Access-Control-Allow-Origin", origin);
    }
...
app.options("/movies/:id", (req: any, res: any) => {
    const origin = req.header("origin");
    if (ACCEPTED_ORIGINS.includes(origin) || !origin) {
        res.header("Access-Control-Allow-Origin", origin);
    }
